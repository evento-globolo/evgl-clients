...
if command -v ruby >/dev/null 2>&1; then ruby -c clients/ruby/lib/evgl_client.rb; fi
if command -v php >/dev/null 2>&1; then php -l clients/php/src/ServiceClient.php; fi
if command -v javac >/dev/null 2>&1; then tmp="$(mktemp -d)"; trap 'rm -rf "$tmp"' EXIT; javac -d "$tmp" clients/java/src/main/java/com/eventoglobolo/client/ServiceClient.java; fi
