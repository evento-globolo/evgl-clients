...
if command -v c++ >/dev/null 2>&1; then
  printf '#include "evgl/client.hpp"\nint main(){evgl::Client c{"https://example.invalid"}; return c.endpoint("events").empty();}\n' |
    c++ -std=c++17 -Wall -Wextra -Werror -Iclients/cpp/include -x c++ - -o "$work/evgl-client-cpp"
else echo 'SKIP c++ compiler'; fi
